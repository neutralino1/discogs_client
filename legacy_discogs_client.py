...
class APIBase(object):
    ...
    def _uri(self):
        ...
        return '%s/%s/%s' % (api_uri, self._end_point, urllib.quote_plus(unicode(self._id).encode('utf-8')))
...
class Artist(APIBase):
    ...
    @property
    def uri(self):
        if not self._object_uri:
            self._object_uri = self.data.get('uri')
    ...
    @property
    def image(self):
        return self.images[0].get('uri150')

    @property
    def images(self):
        if not self._images:
            self._images = self.data.get('images')
        return self._images
...
class Search(APIBase):
    def __init__(self, q='', type='', page=1):
        self._id = q
        self._results = {}
        self._exactresults = []
        self._page = page
        APIBase.__init__(self, 'database/search')
    ...
    @property
    def _uri(self):
        return '%s/%s' % (api_uri, self._end_point)
